...
1. Query completed tasks from the past week:
   ...
   SELECT * FROM tasks
   WHERE status = 'done'
   AND last_added_at >= datetime('now', '-7 days')
...
2. Query tasks that were skipped or rolled over:
   ...
   SELECT * FROM tasks
   WHERE status = 'open'
   AND first_added_at < datetime('now', '-7 days');
   ```
...
4. Guide the reflection:
   ...
   **Energy Check**
   ...
   Save energy rating:
   ```sql
   INSERT INTO reviews (id, week_start, energy_reflection, created_at)
   VALUES (lower(hex(randomblob(16))), date('now', 'weekday 0', '-7 days'), ?, datetime('now'));
   ```
